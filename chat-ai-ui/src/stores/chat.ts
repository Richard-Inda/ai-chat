...
export const useChatStore = defineStore('chat', () => {
    ...
    const sendMessage = async (message: string) => {
        ...
        try {
            const { data } = await axios.post(`${import.meta.env.VITE_API_URL}/chat`, { message, userId: userStore.userId });

            messages.value.push({ role: 'ai', content: data.reply });
        } catch (error) {
            console.error('Error sending message:', error);
            messages.value.push({ role: 'ai', content: 'Error processing request' });
        } finally {
